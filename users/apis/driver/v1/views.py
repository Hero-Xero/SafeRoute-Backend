...
from users.services.otp_services import DriverOtpService

class SendOtpView(APIView):
    """
    Validates the Driver credentials (email and password) and sends an OTP to their email.
    """
    ...
    def post(self, request, *args, **kwargs):
        ...
        if user.type != 'DRIVER':
            return Response({"detail": _("User is not a Driver.")}, status=status.HTTP_403_FORBIDDEN)
        ...
        otp_service = DriverOtpService(user)
        otp, created, remaining_time = otp_service.generate_or_get_otp()
        
        # Real-world apps might not return the OTP, but returning for debugging/testing
        return Response({
            "detail": _("OTP has been sent to your email."),
            "remaining_time": remaining_time,
            # "dev_otp_hint": otp  # Usually omit this in production
        }, status=status.HTTP_200_OK)
...
class VerifyOtpView(APIView):
    """
    Verifies the given OTP and returns JWT access and refresh tokens for Drivers.
    ...
    def post(self, request, *args, **kwargs):
        ...
        try:
            ...
            return Response({"detail": _("No Driver Account found with this email.")}, status=status.HTTP_404_NOT_FOUND)
...
class ResendOtpView(APIView):
    """
    Re-validates the Driver credentials and forces a new OTP sending via email.
    """
    authentication_classes = []
    permission_classes = []
...
class SetNewPasswordView(APIView):
    """
    Allows an authenticated Driver to change their password securely.
    """
    permission_classes = [IsAuthenticated]

    def post(self, request, *args, **kwargs):
        user = request.user
        if getattr(user, 'type', None) != 'DRIVER':
            return Response({"detail": _("User is not a Driver.")}, status=status.HTTP_403_FORBIDDEN)
...
class SetInitialPasswordView(APIView):
    """
    Allows a newly registered Driver to set their initial password using the reset token.
